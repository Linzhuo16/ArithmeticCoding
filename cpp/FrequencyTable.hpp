...
class FlatFrequencyTable final : public FrequencyTable {
	...
	private: mutable std::vector<std::uint32_t> frequencies;

	// Binary Index Tree
	private: mutable std::vector<std::uint32_t> BITVec;


	// Always equal to the sum of 'frequencies'.
	private: mutable std::uint32_t total;
	...
	private: void update(uint32_t symbol, uint32_t delta) ;
	// BITVec prefixSum
	private: uint32_t prefixSum(std::uint32_t symbol) const ;
